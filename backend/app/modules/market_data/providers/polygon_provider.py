...
class PolygonProvider(MarketDataProvider):
    ...
    def __init__(self, api_key: str | None = None):
        ...
        self.api_key = api_key or settings.polygon_api_key or settings.market_data_api_key

    def get_latest_price(self, symbol: str) -> PriceQuote:
        self._ensure_api_key()
        raise ExternalProviderError("Polygon/Massive market data provider is not implemented yet")
    ...
    def _ensure_api_key(self) -> None:
        if not self.api_key:
            raise ExternalProviderError(
                "Polygon/Massive market data provider requires POLYGON_API_KEY or MARKET_DATA_API_KEY"
            )
